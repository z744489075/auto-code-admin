...
public class ManyToManyRoleController  {
	...
	/**
	 * 根据条件查询(所有的实体属性都是条件,如果为空则忽略该字段)->角色
	 */
	@ResponseBody
	@Auth("manyToManyRole/selectAllByPaging")
	...
	/**
	 * 分页查询 (所有的��体属性都是条件,如果为空则忽略该字段) (详见Page类.所以的实体都继承该类 默认 page=1 pageSize=10)->角色
	 */
	@ResponseBody
	@Auth("manyToManyRole/selectAllByPaging")
	...
	public void export(ManyToManyRole manyToManyRole,HttpServletRequest request,HttpServletResponse response){
		...
		ExcelUtils.exportExcel("角色",header,list,response,request);
