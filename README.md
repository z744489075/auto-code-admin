# auto-code-admin
欢迎使用auto-code-admin后台代码自动生成引擎.2.0重大升级.支持`单表`, `一对一`, `一对多` ,`多对多`代码生成 [演示地址](http://www.zengtengpeng.com/login/gotoLogin) 账号 `ztp`  密码 `111111`

### 项目介绍 
#### 项目的优势在哪里

> 目前市面上的代码生成工具绝大多数仅仅支持生成单表,该项目支持 `单表`, `一对一`, `一对多` ,`多对多` 
代码生成.大大简化了开发的工作量

#### 什么情况选择该项目

> 1.该项目接口和页面一起生成.如果你要做后台系统.该项目将大大节省你的工作量,只需要专注实现业务逻辑.
> 1.该项目只生成接口(controller,service,serviceImpl,dao,xml),
不生成页面.所以如果项目是采用前后台分离,不需要写页面.该项目会适合你

> 2.如果你仅仅只是想生成接口.不需要生成页面(前后台分离).请使用该项目 [源码地址](https://gitee.com/ztp/auto-code).
 
 
#### 为何会发起该项目? (由于线上的数据被恶意删除,修改.所以目前每隔一个小时我会恢复一次数据)

    1.权限,角色,用户对于绝大多数系统来说都是一成不变的.而我们每开发一个后台都需要重新一次.每次都在重复造轮子
    2.对于业务绝大多数也是围绕这增删改查来进行操作.每次创建一张表.然后我们需要重新写一次增删改查,写虽然简单,
    不过极度耗时(view,controller,server,serverImpl,dao,xml) 
